...
chrome.contextMenus.create({
    ...
    onclick: function(){
        if (status == 'true') {
            chrome.contextMenus.update('LeakFinder', {title: '开启LeakFinder'} , function(){});
            status = false;
        }else{
            chrome.contextMenus.update('LeakFinder', {title: '关闭LeakFinder'} , function(){});
            status = true;
        }
    }
});
...
function startScan(url) {
    ...
    if(null != whiteList && '' != whiteList && 'undefined' != whiteList){
        whiteListArray = whiteList.split('\n');
        for(var i = 0; i < whiteListArray.length; i++){
            var thisHost = whiteListArray[i].trim();
            if(thisHost == host){
                return;
            }
        }
